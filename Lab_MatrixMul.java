...
public class Lab_MatrixMul{
    public static void main(String[] args) {
        ...
        int matC_c = matB.data[0].length; //findSizeOfMatrix
        MyData matC = new MyData(matC_r, matC_c);
        //Q4
        Thread[] t = new Thread[matC_r*matC_c];
        int tIndex= 0;
        for (int row = 0; row < matC.data.length; row++){
            for (int col = 0; col < matC.data[0].length; col++){
            t[tIndex] = new Thread(new MatrixMulThread(row, col, matA, matB, matC));
                t[tIndex].start();
                tIndex++;
            }
        }
